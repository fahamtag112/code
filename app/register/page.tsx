...
export default function RegisterPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      // success -> redirect to home or protected page
      router.push('/');
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6">
      <div className="max-w-md w-full rounded-2xl bg-white/5 p-8 border border-white/6">
        ...
        <div className="mt-4 text-sm text-slate-400">Already have an account? <a className="underline" href="/login">Log in</a></div>
      </div>
    </div>
  );
}
